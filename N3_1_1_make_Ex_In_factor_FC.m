...
for i=1:4
       ...
       % BL       
       eval('Across_FC.BL_ex_pp_only_FC_sum{i} = sum(fc_bl(BL_subject_all(:,i),exter_pospos_type{1,i}),2);');
       eval('Across_FC.BL_in_nn_only_FC_sum{i} = sum(fc_bl(BL_subject_all(:,i),inter_negneg_type{1,i}),2);');
       
       % FU2
       eval('Across_FC.FU2_ex_pp_only_FC_sum{i} = sum(fc_fu2(FU2_subject_all(:,i),exter_pospos_type{1,i}),2);');
       eval('Across_FC.FU2_in_nn_only_FC_sum{i} = sum(fc_fu2(FU2_subject_all(:,i),inter_negneg_type{1,i}),2);');
       
       % FU2_Rest
       eval('Across_FC.FU2_Rest_ex_pp_only_FC_sum{i} = sum(fc_fu2_rest(:,exter_pospos_type{1,i}),2);');
       eval('Across_FC.FU2_Rest_in_nn_only_FC_sum{i} = sum(fc_fu2_rest(:,inter_negneg_type{1,i}),2);');
       
       % FU3
       eval('Across_FC.FU3_ex_pp_only_FC_sum{i} = sum(fc_fu3(FU3_subject_all(:,i),exter_pospos_type{1,i}),2);');
       eval('Across_FC.FU3_in_nn_only_FC_sum{i} = sum(fc_fu3(FU3_subject_all(:,i),inter_negneg_type{1,i}),2);');
...
save N3_1_1_make_Ex_In_factor_FC TableS3_1_1_make_factor_fc
